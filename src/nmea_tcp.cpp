...
int main(int argc, char** argv)
{
  ...
  // Read parameters
  node_handle_.param("/nmea_tcp_driver/address", address, std::string("127.0.0.1"));
  node_handle_.param("/nmea_tcp_driver/port", port, 28003);
  node_handle_.param("/nmea_tcp_driver/nmea_topic", nmea_topic, std::string("nmea_sentence"));
  node_handle_.param("/nmea_tcp_driver/frame_id", frame_id, std::string("sentence"));
  node_handle_.param("/nmea_tcp_driver/rate", rate, 10.0);

  pub = node_handle_.advertise<nmea_msgs::Sentence>(nmea_topic, 10);

  ROS_INFO("IP: %s", address.c_str());
  ROS_INFO("PORT: %d", port);
  ROS_INFO("RATE: %d", port);
